...
function IconButton({ label, onClick, disabled, children }: any) {
  return (
    <button
      ...
      onClick={onClick}
      disabled={disabled}
      className="grid h-11 w-11 place-items-center rounded-full text-white/75 transition hover:bg-white/10 hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#D4AF37] disabled:opacity-35 disabled:hover:bg-transparent"
    >
      {children}
    </button>
  );
...
export default function AdminBuilderHeader({
  ...
  return (
    <header className="flex min-h-16 shrink-0 flex-wrap items-center justify-between gap-x-2 border-b border-[#D4AF37]/25 bg-[#303839] px-3 py-2 shadow-[0_8px_24px_rgba(48,56,57,0.16)] xl:h-16 xl:flex-nowrap xl:py-0 2xl:px-5">
      {/* Left */}
      ...
      {/* Centre tabs */}
      <nav className="order-3 flex h-11 w-full shrink-0 items-stretch overflow-x-auto no-scrollbar xl:order-none xl:h-full xl:w-auto" aria-label="Builder sections">
        {TABS.map((t) => {
