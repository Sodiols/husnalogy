...
function Stepper({ label, value, step = 1, minimum, maximum, onChange }: { label: string; value: number; step?: number; minimum?: number; maximum?: number; onChange: (value: number) => void }) {
  return (
    <div>
      ...
      <EditableNumericStepper label={label} value={Number(value) || 0} minimum={minimum} maximum={maximum} step={step} largeStep={step < 1 ? step * 10 : 10} allowNegative={minimum === undefined || minimum < 0} allowDecimal={step < 1} onCommit={onChange} className="h-11 w-full rounded-xl border border-[#303839]/12 bg-white" />
    </div>
  );
}
  ...
  return (
    <div className="grid gap-2 rounded-xl border border-[#303839]/10 bg-[#F8F6F1] p-3">
      ...
      <input ref={inputRef} type="file" accept="image/png,image/webp" className="sr-only" onChange={async (event) => {
        ...
        setBusy(true);
        try {
          const asset = await uploadBuilderImage(file, overlay.type === "texture" ? "texture" : "overlay");
          if (asset.url) onPatch({ assetId: asset.id, src: asset.editorUrl || asset.url });
        }
        ...
        <aside className="grid content-start gap-4">
          ...
          <section className="rounded-2xl border border-[#303839]/10 bg-white p-4 shadow-sm">
            ...
            <div className="mt-3 grid grid-cols-2 gap-2">
              ...
              <Stepper label="Width" value={area.width} minimum={1} onChange={(width) => patchArea({ width })} /><Stepper label="Height" value={area.height} minimum={1} onChange={(height) => patchArea({ height })} />
              <Stepper label="Rotation" value={area.rotation || 0} minimum={-360} maximum={360} step={0.5} onChange={(rotation) => patchArea({ rotation })} /><Stepper label="Opacity" value={area.opacity ?? 1} minimum={0} maximum={1} step={0.05} onChange={(opacity) => patchArea({ opacity })} />
            </div>
