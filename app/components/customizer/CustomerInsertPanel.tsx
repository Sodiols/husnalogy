...
export default function CustomerInsertPanel({ tool, onAddShape, onAddLine, onAddFrame, onAddGrid, onAddQRCode, onSetBackground, allowedShapes = [], allowedFrameMasks = [], allowedGridPresets = [], allowedColors = [] }: any) {
  ...
  if (tool === "shapes") return <div className="grid grid-cols-2 gap-2 p-4">{["rectangle", "rounded-rectangle", "circle", "oval", "triangle", "polygon", "arch"].filter((shape) => permit(allowedShapes, shape)).map((shape) => <button key={shape} type="button" onClick={() => onAddShape(shape)} className={button}>{shape.replace("-", " ")}</button>)}</div>;
  if (tool === "lines") return <div className="grid gap-2 p-4">{["solid", "dashed", "dotted"].map((style) => <button key={style} type="button" onClick={() => onAddLine(style)} className={button}>{style} line</button>)}</div>;
