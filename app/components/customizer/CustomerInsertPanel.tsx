...
export default function CustomerInsertPanel({ tool, onAddShape, onAddLine, onAddFrame, onAddGrid, onAddQRCode, onSetBackground, allowedShapes = [], allowedFrameMasks = [], allowedGridPresets = [], allowedColors = [], allowShapes = true, allowLines = true }: any) {
  ...
  // Shapes and lines share ONE panel. Each half is gated by its own template
  // permission, so a design that allows shapes but not lines shows only shapes.
  if (tool === "shapes") {
    const shapes = allowShapes
      ? ["rectangle", "rounded-rectangle", "circle", "oval", "triangle", "polygon", "arch"].filter((shape) => permit(allowedShapes, shape))
      : [];
    const lineStyles = allowLines ? ["solid", "dashed", "dotted"] : [];
    if (!shapes.length && !lineStyles.length) return null;
    return (
      <div className="grid gap-3 p-4">
        {shapes.length ? (
          <div className="grid grid-cols-2 gap-2">
            {shapes.map((shape) => (
              <button key={shape} type="button" onClick={() => onAddShape(shape)} className={button}>{shape.replace("-", " ")}</button>
            ))}
          </div>
        ) : null}
        {lineStyles.length ? (
          <div className="grid gap-2">
            {shapes.length ? (
              <p className="text-[10px] font-extrabold uppercase tracking-[0.12em] text-[#303839]/45">Lines</p>
            ) : null}
            <div className="grid grid-cols-2 gap-2">
              {lineStyles.map((style) => (
                <button key={style} type="button" onClick={() => onAddLine(style)} className={button}>{style} line</button>
              ))}
            </div>
          </div>
        ) : null}
  ...
  if (tool === "frames") return <div className="grid grid-cols-2 gap-2 p-4">{["rectangle", "rounded", "circle", "oval", "arch", "arch-top", "arch-bottom"].filter((shape) => permit(allowedFrameMasks, shape)).map((shape) => <button key={shape} type="button" onClick={() => onAddFrame(shape)} className={button}>{shape.replace("-", " ")} frame</button>)}</div>;
