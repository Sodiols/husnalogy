    ...
    <div className="fixed inset-0 z-[180] grid place-items-center bg-[#303839]/45 p-4" onPointerDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
      <section role="dialog" aria-modal="true" aria-labelledby="shortcut-help-title" className="max-h-[85vh] w-full max-w-xl overflow-y-auto rounded-3xl border border-white/40 bg-white p-5 shadow-[0_28px_90px_rgba(48,56,57,0.28)] sm:p-7">
        <div className="flex items-start justify-between gap-4">
          ...
          <button ref={closeRef} type="button" onClick={onClose} className="min-h-11 rounded-xl border border-[#303839]/12 px-4 text-xs font-extrabold text-[#303839] hover:bg-[#303839]/5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#D4AF37]">Close</button>
        </div>
        <dl className="mt-6 grid gap-2 sm:grid-cols-2">
          {shortcuts.map(([label, keys]) => <div key={label} className="rounded-2xl bg-white p-4"><dt className="text-xs font-extrabold text-[#303839]">{label}</dt><dd className="mt-2 text-[11px] font-semibold leading-5 text-[#303839]/55">{keys}</dd></div>)}
        </dl>
      </section>
    </div>
  );
