...
export async function DELETE(_request: Request, { params }: any) {
  ...
  // Remove the authoritative row first, then Storage. If Storage cleanup
  // fails, restore the row so the library never intentionally points at
  // missing objects and the user can safely retry.
  ...
  if (removePaths.length) {
    ...
    if (storageError) {
      const { error: restoreError } = await service.from("customer_asset_library").insert(asset);
      console.error("Customer asset storage cleanup failed:", storageError, "record restore:", restoreError);
      return Response.json({ ok: false, error: "Storage cleanup failed; your photo was restored. Please retry." }, { status: 500 });
    }
  }
