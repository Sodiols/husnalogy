...
- Storage: private `customizer-elements` bucket; `customizer_assets` is the
  authoritative metadata table and display APIs generate fresh signed URLs
  from permanent original/editor/thumbnail paths. Categories and folders are
  organization metadata, not duplicate asset stores.
...
Buckets: `customizer-elements` (private, admin-managed + signed URLs),
`customizer-renders` (private, service-role only + signed URLs).
...
Apply all timestamped Customizer V2 migrations before enabling feature flags.
Required buckets are `customer-uploads` (private), `customizer-elements` (private),
