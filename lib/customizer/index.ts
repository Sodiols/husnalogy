...
function normalizeTextStyle(input: any = {}): any {
  ...
  return {
    ...
    fitMode:
      cleanString(input.fitMode) === "shrink"
        ? "shrink"
...
export function normalizeCustomizerLayer(input: any = {}): any {
  ...
  if (type === "image" || type === "frame") {
    ...
    return {
      ...
      originalPath: cleanOptionalString(input.originalPath || input.path),
      ...
      placeholderAssetId: cleanOptionalString(input.placeholderAssetId),
      placeholderAssetPath: cleanOptionalString(input.placeholderAssetPath),
      placeholderAssetBucket: cleanOptionalString(input.placeholderAssetBucket),
      placeholderAssetEditorPath: cleanOptionalString(input.placeholderAssetEditorPath),
      placeholderAssetThumbnailPath: cleanOptionalString(input.placeholderAssetThumbnailPath),
...
export function normalizeCustomizerPage(input: any = {}, index = 0): any {
  ...
  return {
    ...
    backgroundAssetId: cleanOptionalString(input.backgroundAssetId),
    bucket: cleanOptionalString(input.bucket),
    originalPath: cleanOptionalString(input.originalPath),
    editorPath: cleanOptionalString(input.editorPath),
    thumbnailPath: cleanOptionalString(input.thumbnailPath),
