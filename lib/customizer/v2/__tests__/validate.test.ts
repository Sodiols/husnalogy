...
describe("server customization validation", () => {
  ...
  it("preserves submitted line breaks when multiline is enabled", () => {
    const multilineTemplate = {
      ...
      layers: template.layers.map((layer) =>
        layer.id === "names_layer" ? { ...layer, textStyle: { multiline: true } } : layer,
      ),
    };
  ...
  it("clamps customer font scaling to the administrator configured range", () => {
    const boundedTemplate = {
      ...template,
      layers: template.layers.map((layer) => layer.id === "names_layer" ? {
        ...layer,
        textStyle: { fontSize: 80, minFontSize: 40, maxFontSize: 100 },
        customerPermissions: { ...layer.customerPermissions, resize: true, changeFontSize: true },
      } : layer),
    };
    const result = validateCustomerState(boundedTemplate, {
      editorState: {
        layerOverrides: { names_layer: { textStyle: { fontSize: 999 } } },
        userLayers: [],
      },
    });
    expect(result.sanitizedEditorState.layerOverrides.names_layer.textStyle.fontSize).toBe(100);
    expect(result.violations.some((violation) => violation.code === "font-size-out-of-range")).toBe(true);
  });
