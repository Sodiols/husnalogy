...
describe("permanent administrator asset references", () => {
  it("stores only IDs and private paths for layers, product images, and page backgrounds", () => {
    const stored: any = stripAdminAssetUrls({
      layer: { assetId: "11111111-1111-4111-8111-111111111111", bucket: "customizer-elements", originalPath: "assets/a/original/a.png", src: "https://expired.test/layer" },
      option: { imageAssetId: "22222222-2222-4222-8222-222222222222", imagePath: "assets/b/original/b.png", image: "https://expired.test/option" },
      page: { backgroundAssetId: "33333333-3333-4333-8333-333333333333", originalPath: "assets/c/original/c.png", backgroundImage: "https://expired.test/page", thumbnail: "https://expired.test/thumb" },
    });
    expect(stored.layer.src).toBeUndefined();
    expect(stored.option.image).toBeUndefined();
    expect(stored.page.backgroundImage).toBeUndefined();
    expect(stored.page.thumbnail).toBeUndefined();
    expect(stored.layer.originalPath).toContain("assets/a/original");
  });

  it("generates fresh signed URLs from the saved bucket and paths", async () => {
    const row = {
      id: "11111111-1111-4111-8111-111111111111",
      title: "Flower",
      bucket: "customizer-elements",
      path: "assets/a/original/a.png",
    ...
    const supabase: any = {
      from: () => query,
      storage: { from: () => ({ createSignedUrl: async (storagePath: string) => ({ data: { signedUrl: `https://fresh.test/${storagePath}?token=${++signCount}` }, error: null }) }) },
    };
