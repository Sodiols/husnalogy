...
export function sniffImageType(buffer: Buffer, allowSvg = false): SniffedImage {
  ...
  return { ok: false, error: allowSvg ? "Unsupported file type. Use SVG, JPG, PNG, or WebP." : "Unsupported file type. Use JPG, PNG, or WebP." };
}
...
const SVG_FORBIDDEN_PATTERNS: Array<{ pattern: RegExp; reason: string }> = [
  ...
  { pattern: /<link[\s>]/i, reason: "external stylesheets" },
  { pattern: /@import\b/i, reason: "imported stylesheets" },
...
// Only same-document fragment ids (for gradients/masks) stay allowed. Data
// URIs can themselves contain SVG/XML and are therefore rejected as nested
// untrusted documents.
const EXTERNAL_REF = /(?:href|xlink:href)\s*=\s*["'](?!#)/i;
