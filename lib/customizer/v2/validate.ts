...
export function validateCustomerState(
  ...
  for (const [layerId, rawOverride] of Object.entries(submittedOverrides)) {
        ...
        if (s.fontSize !== undefined) {
          ...
          if (clampedFontSize !== s.fontSize) {
            violations.push({
              ...
              layerId,
              message: `Font size on "${layer.name || layerId}" must stay between ${minFontSize} and ${maxFontSize}.`,
            });
          }
    ...
    if (override.properties) {
      ...
      const textKeys = new Set(["text"]);
      for (const [key, value] of Object.entries(override.properties)) {
        const allowed = layer.type === "text"
          ? (textKeys.has(key) && permissions.editContent)
          : layer.type === "shape"
