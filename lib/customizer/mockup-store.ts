...
export async function saveNormalizedMockupTemplate(productId: string, template: MockupTemplate, publish = false): Promise<MockupTemplate> {
  ...
  const { error } = await supabase.rpc("upsert_customizer_mockup", { p_product_id: productId, p_payload: stripAdminAssetUrls(template), p_publish: publish });
  if (error) throw error;
