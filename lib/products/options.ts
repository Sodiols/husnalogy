...
export function parseProductOption(entry: ProductOptionEntry): ParsedProductOption | null {
  ...
  return {
    ...
    imageAssetId: cleanText(entry.imageAssetId, 80),
    imageBucket: cleanText(entry.imageBucket, 100),
    imagePath: cleanText(entry.imagePath, 1000),
    imageEditorPath: cleanText(entry.imageEditorPath, 1000),
    imageThumbnailPath: cleanText(entry.imageThumbnailPath, 1000),
  ...
  value.forEach((entry) => {
    ...
    if (entry && typeof entry === "object") {
      ...
      const rich: RichProductOption = {
        ...
        imageAssetId: cleanText((entry as any).imageAssetId, 80),
        imageBucket: cleanText((entry as any).imageBucket, 100),
        imagePath: cleanText((entry as any).imagePath, 1000),
        imageEditorPath: cleanText((entry as any).imageEditorPath, 1000),
        imageThumbnailPath: cleanText((entry as any).imageThumbnailPath, 1000),
